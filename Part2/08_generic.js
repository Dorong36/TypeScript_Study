...
var firstValue = arrFirst([1, 2, 3, 4]); // 결과는 1
...
// 확장성이 생김
var myFirstVal1 = genericFun([6, 3, 7, 1]); // 타입 파라미터를 넣어줌
var myFirstVal2 = genericFun(['2', '3', '5']);
...
var minusVal = minusOne(30);
// string 으로 extends확인해서 length 기능도 쓸 수 있고~~
function strLen(data) {
    return data.length;
}
;
function namePrint208(data) {
    return data.name;
}
var name208 = namePrint208({ name: 'yu' });
// class에서도 사용가능
// 예시)
...
function count208(data) {
    return data.length;
}
function toAnimal(data) {
    return JSON.parse(data);
}
var animalData = '{"name" : "dog", "age" : 1}';
var result208 = toAnimal(animalData);
 ...
 */
// Q3
var RanName = /** @class */ (function () {
    function RanName(name) {
        this.name = name;
    }
    return RanName;
}());
